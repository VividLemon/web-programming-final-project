...
if($_SERVER['REQUEST_METHOD'] == "GET"){
	...
	if(empty($validationErrors)){
		...
		// Create a thumbnail of the image 
			...
			$resize_width = 150;								
			$resize_height = 200;
			$resize_for_blog_default = 300;
			$newFileNameForDefaults = $file['fileId'] . "." . $file['fileExtension'];
			$newFilePathForDefaults = SERVER_UPLOAD_FOLDER . "defaults/" . $newFileNameForDefaults;
			$resize_result = $imgUploader->resizeImage($newFilePath, $thumbNailPath, $resize_width, $resize_height);
			$resize_default_result = $imgUploader->resizeImage($newFilePath, $newFilePathForDefaults, $resize_for_blog_default, $resize_for_blog_default);
			// resize the image...
		
			// $resize_result will be an array that has some details about the final size of the image
			if($resize_result ==  false || $resize_default_result == false){
