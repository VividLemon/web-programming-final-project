...
require("../includes/CategoryDataAccess.inc.php");
...
if($_SERVER['REQUEST_METHOD'] == "GET"){
	...
	$page['defaultImg'] = $_POST['defaultImg'];
...
<script>
	window.addEventListener("load", () => {
		...
		document.getElementById("btnInsertImg").addEventListener("click", function () {
			createWindow(400, 300, true);
		});

		document.getElementById("btnDefaultImg").addEventListener("click", function () {
			createWindow(400, 300, false); // bad way of doing this.. but works for it's purpose.. Cannot modify easily.
		});

		// STEP 3
		// create a pop up window that displays the image-list page
		// (and handles clicks on the A tags in the image list page)
		function createWindow(width, height, isInsertingImage) {
			...
			popup.addEventListener("click", function (evt) {
				...
				if (evt.target.classList.contains("insertImg")) {
					...
					if(isInsertingImage){ // tells whether it is supposed to insert image into text, or post data
						insertImg(imgName, imgDesc);
					}else{
						document.getElementById(`defaultImg`).value = `${imageFolderPath}${imgName}`;
					}
...
<main>
	<div class="wrapper style1">
		...
		<form method="POST" action="<?php echo($_SERVER['PHP_SELF']) ?>">
			...
			<label>Title<?php echo(isset($validationErrors['title']) ? wrapValidationMsg($validationErrors['title']) : ""); ?></label>
			<input type="text" name="title" value="<?php echo($page['title']); ?>" />
			<label>Description<?php echo(isset($validationErrors['description']) ? wrapValidationMsg($validationErrors['description']) : ""); ?></label>
			<textarea name="description"><?php echo($page['description']); ?></textarea>
			<label>Content<?php echo(isset($validationErrors['content']) ? wrapValidationMsg($validationErrors['content']) : ""); ?></label>
			<textarea name="content"><?php echo($page['content']); ?></textarea>
			<br>
			<input type="button" value="Insert Image" id="btnInsertImg" />
			<label>Published Date (mm/dd/yyyy)<?php echo(isset($validationErrors['publishedDate']) ? wrapValidationMsg($validationErrors['publishedDate']) : ""); ?></label>
			<input name="publishedDate" value="<?php echo($page['publishedDate']); ?>" />
			<label>Category<?php echo(isset($validationErrors['categoryId']) ? wrapValidationMsg($validationErrors['categoryId']) : ""); ?></label>
			...
			<input type="button" value="Choose default image" id="btnDefaultImg" style="margin-top:1rem;">
			<input type="hidden" name="defaultImg" value="<?php echo(PROJECT_DIR . "uploaded-files/" . DEFAULT_IMG) ?>" id="defaultImg">
			<label>Active<?php echo(isset($validationErrors['active']) ? wrapValidationMsg($validationErrors['active']) : ""); ?></label>
